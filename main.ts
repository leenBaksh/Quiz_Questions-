...
console.log(
  chalk.redBright.underline(
    `\t\tWelcome ${user.name} to the TypeScript Quiz!`
  )
);
console.log(
  chalk.magentaBright.underline("\t\t---------Please provide the most accurate response you can to the following questions.----------")
);
console.log(chalk.bgCyanBright.underline("\t\tLet's start the quiz!"));
...
const quiz: {
...
} = await inquirer.prompt([
  {
    ...
    message: chalk.yellowBright.bold(
      "Q1. What is the TypeScript file extension?"
    ),
  ...
  },
  {
    ...
    message: chalk.blueBright.bold(
      "Q2. The code in TypeScript is compiled into:"
    ),
    choices: [
      "Machine code",
      "Assembly code",
  ...
  },
  {
    ...
    type: "list",
    message: chalk.blueBright.bold(
      "Q3. What is the most popular tool for converting TypeScript code to JavaScript?"
    ),
...
console.log(
  chalk.redBright.bold.underline(
    `\t\t${user.name} Your score is ${score}/10`
  )
);
console.log(
  chalk.redBright.bgWhiteBright.bold("\t\tWe appreciate you taking the test!")
);
